            ...
            .then(valid => {
                ...
                if (!valid) {
                    ...
                    res.status(200).json({ //S'ils correspondent, les informations d'identification de notre utilisateur sont valides. Dans ce cas, nous renvoyons une réponse 200 contenant l'ID utilisateur et un token
                        ...
                        userId: user._id,
                        token: jwt.sign( // Les tokens d'authentification permettent aux utilisateurs de se connecter une seule fois à leur compte, sign de jsonwebtoken pour chiffrer un nouveau token
                        { userId: user._id },
